...
| `images/logo.png` | 327x160 RGBA wordmark, the nav brand on `index.html` |
...
`icon.svg` is **not vector**. It is a 512x512 `<svg>` wrapping one
`data:image/png;base64` payload, zero `<path>` elements, 77 KB. It is valid and
...
The nav brand is written by `build.js`, not by hand, and `nav.brand` from the
CMS still supplies the `alt` text. The three pages under `case-studies/` are
static and still carry the old text wordmark.
